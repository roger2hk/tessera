...
// $ docker run --name test-postgresql -p 5432:5432 -e POSTGRES_PASSWORD=root -e POSTGRES_DB=test_tessera -d postgres
...
import (
	...
	"github.com/transparency-dev/trillian-tessera/storage/postgresql"
...
func TestNew(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			_, err := postgresql.New(ctx, testDB, test.opts...)
			gotErr := err != nil
...
func TestReadMissingTile(t *testing.T) {
	ctx := context.Background()
	s := newTestPostgreSQLStorage(t, ctx)
...
func newTestPostgreSQLStorage(t *testing.T, ctx context.Context) *postgresql.Storage {
	t.Helper()

	s, err := postgresql.New(ctx, testDB, tessera.WithCheckpointSignerVerifier(noteSigner, noteVerifier))
	if err != nil {
		t.Errorf("Failed to create postgresql.Storage: %v", err)
	}
